(function (){
    ...
    module.exports = this.DataHandler = new class {
        ...
        responseRenderer = async function (msg, callback) {
            if (msg === '/start' || msg === 'Назад')
                ...
                await callback(getResponseData(msg, async function (a) {
                    if (!a){
                        await callback('Не найден возможный ответ')
                    }else {
        ...
        callPhotoFromRenderer = async function (msg, callback) {
            await getResponsePhoto(msg, function (a) {
                callback(a);
            })
        }
    }
    ...
    function clearText(val) {
        try {
            val.toString()
            try {
                return val.toString().trim() //.replace(/^a-zA-Z0-9 ]/g, '')
            }catch (e) {
                return val
            }
        }
    ...
    async function createList2(data, callback) {
        ...
        function callData(el, callback) {
            ...
            xhr.onload = async () => {
                ...
                let finalListData = [{
                    [el]: {
                        "title": data_fcol.map(v => clearText(v)),
                        "content": data_fcol2.map(v => clearText(v)),
                        "file": data_fcol3.map(v => clearText(v)),
                        "media": data_fcol4.map(v => clearText(v)),
                        "image": data_fcol5.map(v => clearText(v)),
                    }
                }]
                data_fcol = []
                data_fcol2 = []
    ...
    async function getResponseMedia(message, callback) { // TODO: More than one response media
        let data_nameFile = 'localResponseData_type_list';
        let raw_data = fs.readFileSync(`${PathData.path_to_local}/${data_nameFile}.json`)
        let parsed_data = JSON.parse(raw_data.toString());
        let i = 0;
    ...
    async function getResponsePhoto(message, callback) { // TODO: More than one response media
        let data_nameFile = 'localResponseData_type_list';
        let raw_data = fs.readFileSync(`${PathData.path_to_local}/${data_nameFile}.json`)
        let parsed_data = JSON.parse(raw_data.toString());
        let i = 0;
        ...
        await parsed_data["response"].forEach(obj => {
            obj[0][Object.keys(obj[0])].title.forEach(el => {
                if (el === message) {
                    r = obj[0][Object.keys(obj[0])[0]].image[obj[0][Object.keys(obj[0])[0]].title.indexOf(el)]
                }
            });
            i++
        })
